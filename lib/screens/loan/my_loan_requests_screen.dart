...
// مسار الملف: lib/screens/my_loan_requests_screen.dart
...
class _MyLoanRequestsScreenState extends State<MyLoanRequestsScreen> {
  ...
  Future<Map<String, dynamic>> _fetchData() async {
    ...
    requests.sort((a, b) => b.reqLoanDate.compareTo(a.reqLoanDate));
    ...
    return Scaffold(
      ...
      body: FutureBuilder<Map<String, dynamic>>(
        ...
        builder: (context, snapshot) {
          ...
          final List<LoanRequest> requests = snapshot.data!['requests'];
...
class _MyLoanRequestsScreenState extends State<MyLoanRequestsScreen> {
  ...
  Future<Map<String, dynamic>> _fetchData() async {
    try {
      final requests = await _loanService.getLoanRequests(widget.user.usersCode.toString());
      final types = await _loanService.getLoanTypes();
      // حفظ البيانات وتطبيق الفلتر المبدئي
      ...
      appBar: AppBar(
        ...
        actions: [
          ...
          IconButton(
            ...
            onPressed: () {
              // التأكد من أن البيانات قد تم تحميلها قبل فتح الفلتر
  ...
  // باقي الدوال (_buildRequestCard, _getStatusInfo, etc.) تبقى كما هي بدون تغيير
  Map<String, dynamic> _getStatusInfo(BuildContext context, int flag) {
    final localizations = AppLocalizations.of(context)!;
    switch (flag) {
      case 1:

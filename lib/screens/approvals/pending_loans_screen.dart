import 'package:asconscai/models/loan_type_model.dart';
...
class PendingLoansScreen extends StatefulWidget {
  ...
  const PendingLoansScreen({super.key, required this.user});
...
class _PendingLoansScreenState extends State<PendingLoansScreen> {
  ...
  Future<Map<String, dynamic>> _fetchData() async {
    final requestsFuture = _approvalsService.getPendingLoanRequests(widget.user.usersCode.toString());
    final typesFuture = _loanService.getLoanTypes();
  ...
  String _getLoanTypeName(int typeCode, List<LoanType> types, bool isRtl) {
    try {
      final type = types.firstWhere((t) => t.loanTypeCode == typeCode);
      return isRtl ? type.nameA : (type.nameE ?? type.nameA);
    } catch (e) {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        title: Text(localizations.translate('approve_loans')!, style: const TextStyle(color: Colors.white, fontWeight: FontWeight.bold, fontSize: 18)),
        actions: [
          IconButton(
      ...
      body: RefreshIndicator(
        ...
        child: FutureBuilder<Map<String, dynamic>>(
          ...
          builder: (context, snapshot) {
            ...
            if (!snapshot.hasData) {
              return EmptyStateWidget(message: localizations.translate('no_pending_loans')!, icon: Iconsax.document_cloud);
            }

            final List<PendingLoanRequest> requests = snapshot.data!['requests'];
  ...
  Widget _buildRequestCard(BuildContext context, PendingLoanRequest request, List<LoanType> types) {
    final isRtl = Localizations.localeOf(context).languageCode == 'ar';
    final empName = isRtl ? request.empName : request.empNameE;
    final loanType = _getLoanTypeName(request.loanType, types, isRtl);
    ...
    return Container(
      ...
      child: Material(
        ...
        child: InkWell(
          onTap: () => _showDetailsSheet(context, request, loanType),
          borderRadius: BorderRadius.circular(20),
          child: Padding(
            ...
            child: Column(
              ...
              children: [
                ...
                Row(
                  children: [
                    Expanded(child: _buildEnhancedInfoChip(Iconsax.calendar_1, DateFormat.yMMMd().format(request.reqLoanDate), Colors.orange)),
                    const SizedBox(width: 12),
                    Expanded(child: _buildEnhancedInfoChip(Iconsax.dollar_circle, request.loanValuePys.toStringAsFixed(2), Colors.green)),
                  ],
                ),
              ],
            ),
  ...
  void _showDetailsSheet(BuildContext context, PendingLoanRequest request, String loanType) {
    final localizations = AppLocalizations.of(context)!;
                  ...
                  child: Column(
                    ...
                    children: [
                              ...
                              child: Column(
                                ...
                                children: [
                                  ...
                                  Container(
                                    ...
                                    child: Text(loanType, style: const TextStyle(fontSize: 12, color: Colors.white, fontWeight: FontWeight.w500)),
                                  ),
                                ],
                              ),
                            ),
                      ...
                      Container(
                        ...
                        child: Column(
                          children: [
                            _buildEnhancedDetailRow(Iconsax.calendar, localizations.translate('request_date')!, DateFormat.yMMMd().format(request.reqLoanDate), Colors.blue),
                            const SizedBox(height: 16),
                            _buildEnhancedDetailRow(Iconsax.money_3, localizations.translate('loan_value')!, request.loanValuePys.toStringAsFixed(2), Colors.green),
                            const SizedBox(height: 16),
                            _buildEnhancedDetailRow(Iconsax.receipt_2, localizations.translate('installment_count')!, request.loanNos.toString(), Colors.purple),
                            const SizedBox(height: 16),
                            _buildEnhancedDetailRow(Iconsax.receipt_item, localizations.translate('installment_value')!, request.loanInstlPys.toStringAsFixed(2), Colors.teal),
                            const SizedBox(height: 16),
                            _buildEnhancedDetailRow(Iconsax.calendar_add, localizations.translate('loan_start_date')!, DateFormat.yMMMd().format(request.loanStartDate), Colors.orange),
                            const SizedBox(height: 16),
  ...
  void _showDecisionDialog(BuildContext context, PendingLoanRequest request) {
  ...
  Future<void> _handleDecision(BuildContext dialogContext, PendingLoanRequest request, String notes, int flag, Function(bool) setLoading) async {
    setLoading(true);
    final localizations = AppLocalizations.of(context)!;
    try {
      final statusData = {
        "users_code": request.usersCode.toString(),
      ...
      await _approvalsService.updateLoanStatus(statusData);

      if (request.lastLevel == 1 || (request.lastLevel == 0 && flag == -1)) {
        final orderData = {
          "emp_code": request.empCode.toString(),
          "req_serial": request.reqSerial.toString(),
          "auth_flag": flag,
        };
        await _approvalsService.updateLoanInOrders(orderData);
      }
      ...
      if (mounted) {
        StatusDialog.show(context, localizations.translate('decision_failed')!, isSuccess: false);
      }
    } finally {

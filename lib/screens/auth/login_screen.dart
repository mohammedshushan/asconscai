...
class _LoginScreenState extends State<LoginScreen> {
                    ...
                    Expanded(
                      child: SingleChildScrollView(
                        child: Column(
                          ...
                          children: [
                            SizedBox(height: size.height * 0.02),
                            Hero(tag: 'user_avatar', child: _buildLogo()),
                            const SizedBox(height: 16),
                            _buildTitles(localizations),
                            const SizedBox(height: 30),
                            _buildLoginForm(localizations),
    ...
    return Column(
      children: [
        TextFormField(
          ...
          enabled: !_isLoading,
          decoration: _inputDecoration(
            hint: localizations.translate('userCodeHint') ?? 'كود المستخدم',
            icon: Icons.supervised_user_circle_rounded,
          ),
          validator: (value) {
            if (value == null || value.isEmpty) {
        ...
        TextFormField(
          ...
          enabled: !_isLoading,
          decoration: _inputDecoration(
            hint: localizations.translate('passwordHint') ?? 'كلمة المرور',
            icon: Icons.lock,
          ).copyWith(
            suffixIcon: IconButton(
              icon: Icon(
  ...
  Widget _buildTitles(AppLocalizations localizations) {
    return Column(
      children: [
        Text(
          localizations.translate('systemTitle') ?? 'نظام موارد بشرية',
          textAlign: TextAlign.center,
          style: GoogleFonts.poppins(
            fontSize: 24,
            fontWeight: FontWeight.bold,
        ...
        const SizedBox(height: 8),
        Text(
          localizations.translate('signInToYourAccount') ?? 'سجل الدخول لحسابك',
          textAlign: TextAlign.center,
          style: GoogleFonts.poppins(
            fontSize: 18,
            color: Colors.grey[600]?.withOpacity(_isLoading ? 0.7 : 1.0),
  ...
  Widget _buildRegisterNow(AppLocalizations localizations) {
    return Row(
      ...
      children: [
        Text(
          localizations.translate('dontHaveAccount') ?? 'لا تملك حساب؟',
          style: GoogleFonts.poppins(
            color: Colors.black.withOpacity(_isLoading ? 0.5 : 0.9),
            fontSize: 14,
          ),
        ...
        TextButton(
          ...
          child: Text(
            localizations.translate('registerNow') ?? 'سجل الآن',
            style: GoogleFonts.poppins(
              color: Colors.black.withOpacity(_isLoading ? 0.5 : 1.0),
              fontWeight: FontWeight.bold,
              fontSize: 14,

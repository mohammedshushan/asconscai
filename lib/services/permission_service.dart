...
class PermissionService {
  ...
  PermissionService({http.Client? client})
  ...
  Future<List<PermissionType>> getPermissionTypes() async {
    return _handleApiCall(() async {
      final response = await _client.get(Uri.parse('$_baseUrl/get_exits_type'));
  ...
  Future<List<PermissionRequest>> getPermissionRequests(String empCode) async {
    return _handleApiCall(() async {
      ...
      if (response.statusCode == 200) {
        ...
        throw Exception(
          'Failed to load permission requests. Status code: ${response.statusCode}',
        );
      }
    });
  }
  ...
  Future<int> getMaxSerial() async {
    return _handleApiCall(() async {
      final response = await _client.get(
        Uri.parse('$_baseUrl/get_all_exist_request'),
      );
      if (response.statusCode == 200) {
